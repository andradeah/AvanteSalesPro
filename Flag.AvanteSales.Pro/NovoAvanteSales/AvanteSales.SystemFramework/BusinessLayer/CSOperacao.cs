    ...
    public class CSOperacoes : CollectionBase
    {
        ...
        public CSOperacoes()
        {
            try
            {
                ...
                sqlQuery.AppendLine("SELECT COD_OPERACAO, DSC_OPERACAO ");
                sqlQuery.AppendLine("      ,COD_OPERACAO_CFO, COD_CFO, SEQ_CFO ");
                sqlQuery.AppendLine("      ,IND_MOVIMENTA_VERBA, IND_VRPEDIDO_ABATER_VERBA, IND_VALIDA_PRECO_MINMAX, IND_PRONTA_ENTREGA ");

                if (CSEmpresa.ColunaExiste("OPERACAO", "COD_CONDICAO_PAGAMENTO"))
                    sqlQuery.AppendLine(" ,COD_CONDICAO_PAGAMENTO ");
                else
                    sqlQuery.AppendLine(" ,0 AS 'CODCONDICAOPAGAMENTO' ");

                sqlQuery.AppendLine("  FROM OPERACAO ");
                sqlQuery.AppendLine(" WHERE IND_ATIVO = 1 ");
                ...
                using (SQLiteDataReader sqlReader = CSDataAccess.Instance.ExecuteReader(sqlQuery.ToString()))
                {
                    while (sqlReader.Read())
                    {
                        ...
                        operacao.COD_CONDICAO_PAGAMENTO = sqlReader.GetValue(9) == System.DBNull.Value ? 0 : sqlReader.GetInt32(9);
