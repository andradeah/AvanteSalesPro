...
namespace AvanteSales.BusinessRules
{
    ...
    /// </summary>
    public class CSPoliticaBroker2014
    {
        #region [ Variáveis ]
        ...
        private class TmpVariaveis
        {
            ...
            public string CDORIGPRD;
            public string CDRANGE;
            public string CDUF;
            public string CDATRIBUTO;
            public string CDGRPCLI6;
        ...
        private int quantidadeGratis;
        private int quantidadeAdicional;
        private int quantidadeUnidadeMedidaProduto = 0;
        private int quantidadeUnidadeMedidaMinima = 0;
        private int quantidadeUnidadeMedidaFree = 0;
        private string unidadeMedidaProduto = "";
        private string unidadeMedidaMinima = "";
        private string unidadeMedidaFree = "";
        ...
        public string DSTABELA_ANT = "";
        public int NMSEQTAB_ANT;
        ...
        public CSPoliticaBroker2014(DateTime data, int clienteSoldto, int clienteShipto, string condicaoPagamento)
        {
            this.data = data;
        ...
        public TmpPricingCons[] CalculaPreco(int produto, string grupoComercializacao, int quantidadeInteira, int quantidadeFracao, decimal percentualDesconto, int quantidadeCaixa)
        {
            ...
            {
                //[Klelbio]
                ////CSGlobal.GravarLog("CSPoliticaBroker2014-CalculaPreco", e.Message + " iteracao " + iteration.ToString(), e.InnerException != null ? e.InnerException.ToString() : "", e.StackTrace);
        ...
        private void SetPosTmpPricingCons(int tmpNMCONDLIN, string tmpCDCONDTYP, string tmpCDCONDUNID, decimal tmpDADO, decimal? tmpVALOR, string tmpCDUNMED, string tmpDSTABELA, int tmpNMSEQTAB, string tmpDSFORMULA)
        {
            ...
            item.CDUNMED = tmpCDUNMED;
            item.DSTABELA = tmpDSTABELA;
            item.NMSEQTAB = tmpNMSEQTAB;
            item.DSFORMULA = tmpDSFORMULA;
        }
        ...
        private void InicializaTabelaVariaveis(int produto, string grupoComercializacao)
        {
            ...
            parametros[07] = new SQLiteParameter("@08PRODUTO", produto.ToString().PadLeft(18, '0'));
            parametros[08] = new SQLiteParameter("@09CDFILFAT", filialFaturadoraCliente);
            parametros[09] = new SQLiteParameter("@10CLIENTE_SOLDTO", clienteSoldto.ToString().PadLeft(10, '0'));
            ...
            //[Klelbio]
            //if (CSEmpresa.ColunaExiste("BRK_EPRODUTO", "CDUNIDPESO"))
            sqlQuery.Append("    ,(ROUND(BRK_EPRODUTO.VLLIQMEBAS,2) / (CASE WHEN BRK_EPRODUTO.CDUNIDPESO = 'KGM' THEN 1000.0 ELSE 1000000.0 END)) AS PESOTOTAL ");
            //else
            //    sqlQuery.Append("    ,ROUND(BRK_EPRODUTO.VLLIQMEBAS,2) / 1000000.0 AS PESOTOTAL ");
            ...
            sqlQuery.Append("        ,BRK_EPRODAVAL.CDORIGPRD ");

            //[Klelbio]
            //if (CSEmpresa.ColunaExiste("BRK_EPRECOA860", "CDCONDTYP"))
            //{
            sqlQuery.Append("        ,SUBSTR(BRK_EPRODUTO.CDCONCOR,1,7) AS CDRANGE ");
            sqlQuery.Append("        ,BRK_ECLIENTBAS.DSUF AS CDUF ");
            sqlQuery.Append("        ,BRK_EPRODORG.CDBASPROD AS CDATRIBUTO ");
            sqlQuery.Append("        ,BRK_ECLIENTE.CDGRPCLI6 AS CDGRPCLI6");
            //}
            //else
            //{
            //    sqlQuery.Append("        ,'' AS CDRANGE ");
            //    sqlQuery.Append("        ,'' AS CDUF ");
            //    sqlQuery.Append("        ,'' AS CDATRIBUTO ");
            //    sqlQuery.Append("        ,'' AS CDGRPCLI6");
            //}
            ...
            sqlQuery.Append("    LEFT JOIN BRK_EUNIDMED ON BRK_EUNIDMED.CDPRD = BRK_EPRODFIL.CDPRD AND BRK_EUNIDMED.CDUNIDMED = BRK_EPRODORG.CDUNIDVEN ");
            sqlQuery.Append("    LEFT JOIN BRK_EPRODAVAL ON BRK_EPRODAVAL.CDPRD = ? AND BRK_EPRODAVAL.CDFILFAT = ? ");
            ...
            using (SQLiteDataReader reader = CSDataAccess.Instance.ExecuteReader(true, sqlQuery.ToString(), parametros))
            {
                while (reader.Read())
                {
                    ...
                    if ((grupoComercializacao.Length > 0 && tmpVariaveis.CDGER0.ToUpper() == grupoComercializacao.ToUpper()) || produto == -1)
                    {
                        ...
                        tmpVariaveis.CDORIGPRD = (reader.GetValue(34) == System.DBNull.Value) ? "" : reader.GetString(34);
                        tmpVariaveis.CDRANGE = (reader.GetValue(35) == System.DBNull.Value) ? "" : reader.GetString(35);
                        tmpVariaveis.CDUF = (reader.GetValue(36) == System.DBNull.Value) ? "" : reader.GetString(36);
                        tmpVariaveis.CDATRIBUTO = (reader.GetValue(37) == System.DBNull.Value) ? "" : reader.GetString(37);
                        tmpVariaveis.CDGRPCLI6 = (reader.GetValue(38) == System.DBNull.Value) ? "" : reader.GetString(38);
        ...
        private void ExecutaRegraEPRGRT()
        {

            quantidadeMinima = 0;
            quantidadeGratis = 0;
            quantidadeAdicional = 0;
            quantidadeUnidadeMedidaProduto = 0;
            quantidadeUnidadeMedidaMinima = 0;
            quantidadeUnidadeMedidaFree = 0;
            unidadeMedidaProduto = "";
            unidadeMedidaMinima = "";
            unidadeMedidaFree = "";

            sqlQuery.Length = 0;
            sqlQuery.Append("  SELECT (T1.QTMINIMA / 1000), T1.CDUNMEDQT, (T1.QTGRATIS / 1000), (T1.QTADICIONAL / 1000), T1.CDUNMED, ");
            sqlQuery.Append("		   T3.VLFATCONV AS UMREZPED,  T3.CDUNIDMED AS CDUNIDMEDPED, ");
            sqlQuery.Append("		   T4.VLFATCONV AS UMREZMIN,  T4.CDUNIDMED AS CDUNIDMEDMIN, ");
            sqlQuery.Append("		   T5.VLFATCONV AS UMREZFREE, T5.CDUNIDMED AS CDUNIDMEDFREE ");
            sqlQuery.Append("    FROM BRK_EPROMGRATIS T1                     ");
            sqlQuery.Append("         INNER JOIN BRK_EPRODORG T2             ");
            sqlQuery.Append("         ON T1.CDPRD      = T2.CDPRD AND        ");
            sqlQuery.Append("            T1.CDGER0     = T2.CDGER0 AND       ");
            sqlQuery.Append("		     T1.CDCANDISTR = T2.CDCANDISTR       ");
            sqlQuery.Append("                                                ");
            sqlQuery.Append("         INNER JOIN BRK_EUNIDMED T3             ");
            sqlQuery.Append("         ON T2.CDPRD     = T3.CDPRD AND         ");
            sqlQuery.Append("            T2.CDUNIDVEN = T3.CDUNIDMED         ");
            sqlQuery.Append("                                                ");
            sqlQuery.Append("         INNER JOIN BRK_EUNIDMED T4             ");
            sqlQuery.Append("         ON T1.CDPRD     = T4.CDPRD AND         ");
            sqlQuery.Append("            T1.CDUNMEDQT = T4.CDUNIDMED         ");
            sqlQuery.Append("                                                ");
            sqlQuery.Append("         INNER JOIN BRK_EUNIDMED T5             ");
            sqlQuery.Append("         ON T1.CDPRD   = T5.CDPRD AND           ");
            sqlQuery.Append("            T1.CDUNMED = T5.CDUNIDMED           ");
            sqlQuery.Append("                                                ");
            ...
            sqlQuery.Append("      AND T1.CDCAMPANHA = ''                    ");
            ...
            using (SQLiteDataReader reader = CSDataAccess.Instance.ExecuteReader(false, sqlQuery.ToString(), parametros))
            {
                if (reader.Read())
                {
                    quantidadeMinima = (reader.GetValue(0) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(0));
                    quantidadeGratis = (reader.GetValue(2) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(2));
                    quantidadeAdicional = (reader.GetValue(3) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(3));

                    quantidadeUnidadeMedidaProduto = (reader.GetValue(5) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(5));
                    unidadeMedidaProduto = (reader.GetValue(6) == System.DBNull.Value) ? "" : reader.GetString(6).Trim();
                    quantidadeUnidadeMedidaMinima = (reader.GetValue(7) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(7));
                    unidadeMedidaMinima = (reader.GetValue(8) == System.DBNull.Value) ? "" : reader.GetString(8).Trim();
                    quantidadeUnidadeMedidaFree = (reader.GetValue(9) == System.DBNull.Value) ? 0 : Convert.ToInt32(reader.GetValue(9));
                    unidadeMedidaFree = (reader.GetValue(10) == System.DBNull.Value) ? "" : reader.GetString(10).Trim();
                }
        ...
        private void ExecutaRegraEPA860()
        {
            sqlQuery.Length = 0;
            sqlQuery.Append(" SELECT T1.VLCONDTYP / 1000.000,T1.CDCONDUNID,0.00 AS 'VLLIMINF', 0.00 AS 'VLLIMSUP', '' AS CDUNMED ");
            sqlQuery.Append(" FROM BRK_EPRECOA860 T1 ");
            sqlQuery.Append(" WHERE T1.CDAPLICAC = 'V' AND ");
            sqlQuery.Append(" T1.CDCONDTYP = ? AND ");
            sqlQuery.Append(" T1.CDGER0 = ? AND ");
            sqlQuery.Append(" T1.CDCANDISTR = ? AND ");
            sqlQuery.Append(" T1.CDUF = ? AND ");
            sqlQuery.Append(" T1.CDGRPCLI = ? AND ");
            sqlQuery.Append(" T1.CDRANGE = ? AND ");
            sqlQuery.Append(" T1.CDMERCADO  IN (?, '') AND ");
            sqlQuery.Append(" T1.CDNEGOCIO  IN (?, '') AND ");
            sqlQuery.Append(" T1.CDCATEG    IN (?, '') AND ");
            sqlQuery.Append(" T1.CDSUBCATEG IN (?, '') AND ");
            sqlQuery.Append(" T1.CDSEGMENTO IN (?, '') AND ");
            sqlQuery.Append(" T1.CDATRIBUTO IN (?, '') AND ");
            sqlQuery.Append(" ? BETWEEN T1.DTVIGINI AND T1.DTVIGFIM ");
            sqlQuery.Append(" ORDER BY T1.CDSEGMENTO DESC, ");
            ...
            parametros[01] = new SQLiteParameter("@02CDGER0", tmpVariaveis.CDGER0);
            parametros[02] = new SQLiteParameter("@03CDCANDISTR", tmpVariaveis.CDCANDISTR);
            parametros[03] = new SQLiteParameter("@04CDUF", tmpVariaveis.CDUF);
            parametros[04] = new SQLiteParameter("@05CDGRPCLI6", tmpVariaveis.CDGRPCLI6);
            parametros[05] = new SQLiteParameter("@06CDRANGE", tmpVariaveis.CDRANGE);
            ...
            parametros[11] = new SQLiteParameter("@11CDATRIBUTO", tmpVariaveis.CDATRIBUTO);
            parametros[12] = new SQLiteParameter("@12DATA", tmpVariaveis.DATA);
                ...
                for (int i = 0; i < tmpPricingCons.Length; i++)
                {
                    if (tmpPricingCons[i] != null)
                    {
                        ...
                        if (tmpPricingCons[i].CDCONDTYP == CDCONDTYP && false)
                        {
                            ...
                            CDUNMED = tmpPricingCons[i].CDUNMED;
                            result = true;
                            break;
                        }
                    }
        ...
        private void ExecutaRegra(string sql, bool temporary, SQLiteParameter[] parametros)
        {
            ...
            if (result != null)
            {
                ...
                CDUNMED = result[4].ToString().Trim();
        ...
        private bool ExecutaNovaCondition()
        {
            ...
            CDUNMED_ANT = CDUNMED;
            DSTABELA_ANT = DSTABELA;
            NMSEQTAB_ANT = NMSEQTAB;
            ...
            if (NMCONDLIN_ANT != NMCONDLIN || !hasNext)
            {
                ...
                if (CDCONDTYP_ANT == "" || DSFORMULA_ANT == "F029" || DSFORMULA_ANT == "F164" || DSFORMULA_ANT == "F320" ||
        ...
        private void AplicaFormulaPasso1()
        {
            ...
            {
                case "F029":
                    passo1VALOR = CalculaFormulaF029();
                    break;
                case "F164":
            ...
            SetPosTmpPricingCons(NMCONDLIN_ANT, CDCONDTYP_ANT, CDCONDUNID_ANT, 0, passo1VALOR, CDUNMED_ANT, DSTABELA_ANT, NMSEQTAB_ANT, DSFORMULA_ANT);
            ROWCOUNT = 1;
        }
        ...
        private void AplicaFormulaPasso2()
        {
            ...
            if (DSFORMULA_ANT == "F015")
            {
                valAux = GetSumTmpPricingCons(783, 783);
                valAux = (valAux.HasValue) ? valAux.Value : 0m;
            ...
            SetPosTmpPricingCons(NMCONDLIN_ANT, CDCONDTYP_ANT, CDCONDUNID_ANT, VALOR, passo2VALOR, CDUNMED_ANT, DSTABELA_ANT, NMSEQTAB_ANT, DSFORMULA_ANT);
        }

        private decimal? CalculaFormulaF029()
        {
            decimal? result = null;
            decimal? val = 0m;
            int? quantUndMin = 0;
            int? quantUndFree = 0;
            int? quantidadeFree = 0;

            try
            {
                /// Conversão para Unidade Minima 
                if (unidadeMedidaProduto == unidadeMedidaMinima)
                    unidadeMedidaMinima = unidadeMedidaProduto;

                /// Conversão para Unidade Free 
                if (unidadeMedidaProduto == unidadeMedidaFree)
                    quantidadeUnidadeMedidaFree = quantidadeUnidadeMedidaProduto;
                else
                {
                    if (unidadeMedidaMinima == unidadeMedidaFree)
                        quantidadeUnidadeMedidaFree = quantidadeUnidadeMedidaMinima;
                }

                if (quantidadeUnidadeMedidaProduto > 0 && quantidadeUnidadeMedidaMinima > 0 && quantidadeUnidadeMedidaFree > 0)
                {
                    /// Quantidade em unidade do produto de venda, não considera fração
                    if (quantidadeUnidadeMedidaProduto == quantidadeUnidadeMedidaMinima)
                        quantUndMin = (quantidadeInteira * quantidadeCaixa) + quantidadeFracao;
                    else
                        quantUndMin = (quantidadeInteira * (quantidadeUnidadeMedidaProduto / quantidadeUnidadeMedidaMinima)) + quantidadeFracao;

                    if (quantidadeUnidadeMedidaProduto == quantidadeUnidadeMedidaFree)
                        quantUndFree = (quantidadeInteira * quantidadeCaixa) + quantidadeFracao;
                    else
                        quantUndFree = (quantidadeInteira * (quantidadeUnidadeMedidaProduto / quantidadeUnidadeMedidaFree)) + quantidadeFracao;

                    /// Quantidade Minima 
                    if (quantUndMin >= (quantidadeMinima * quantidadeUnidadeMedidaMinima))
                    {
                        val = GetSumTmpPricingCons(NMCONDINI_ANT, NMCONDFIM_ANT) * -1;
                        quantidadeFree = (quantUndFree / (quantidadeGratis * quantidadeUnidadeMedidaFree)) * (quantidadeAdicional * quantidadeUnidadeMedidaFree);
                        result = (val / quantUndFree) * quantidadeFree;
                    }
                    else
                        result = 0;
                }
        ...
        private void PreprocessaCliente()
        {
            ...
            bool primeiroCliente = false;

            //[Klelbio]
            //if (CSEmpregados.Current.USAR_CACHE_PRICE)
            primeiroCliente = (CSDataAccess.Instance.DisposeCachedResults(TypedHashtable.HashtableEntryType.All) == 0);
        ...
        {
            sqlQuery.Length = 0;
            sqlQuery.Append("SELECT T1.VLCONDTYP / 1000.000, T1.CDCONDUNID, T1.VLLIMINF, T1.VLLIMSUP ,T1.CDUNMED, T1.CDUNMED ");
        ...
        {
            sqlQuery.Length = 0;
            sqlQuery.Append("SELECT T1.VLCONDTYP / 1000.000,T1.CDCONDUNID,T1.VLLIMINF,T1.VLLIMSUP,T1.CDUNMED ");
                    ...
                    case 90:
                        ...
                        if (tmpVariaveis.CDGER0 == "BRG1")
                            ...
                            parametros[01] = new SQLiteParameter("@02CDDINAMIC1", "BR24");
                                ...
                                if (tmpVariaveis.CDGER0 == "BRB3")
                            parametros[01] = new SQLiteParameter("@02CDDINAMIC1", "BR44");
                        else
        ...
        private void ExecutaRegraEPA291()
        {
            ...
            sqlQuery.Append("? BETWEEN T1.DTVIGINI AND T1.DTVIGFIM  AND ");
            sqlQuery.Append(" (( ? IN ('1','2','3') AND T1.CDIMPOSTO = 'I3') OR ");
            sqlQuery.Append("  ( ? NOT IN ('1','2','3') AND T1.CDIMPOSTO = 'C3')) ");
            sqlQuery.Append(" ORDER BY T1.NMCONDREC DESC ");
            ...
            parametros[03] = new SQLiteParameter("@04IDCLIIMP", tmpVariaveis.IDCLIIMP);
        ...
        {
            sqlQuery.Length = 0;
            sqlQuery.Append("SELECT (T1.VLCONDTYP * -1) / 1000,T1.CDCONDUNID,T1.VLLIMINF,T1.VLLIMSUP, T1.CDUNMED ");
        ...
        private void ExecutaRegraEPA850()
        {
            ...
            switch (CDCONDSEQ)
            {
                case "ZSTI":
                    ...
                    else if (NMSEQTAB == 2 || NMSEQTAB == 30)
                    {
                        parametros[03] = new SQLiteParameter("@04CDDINAMIC1", tmpVariaveis.CDCLASFISC);
                        parametros[04] = new SQLiteParameter("@05CDDINAMIC2", "X");
                        parametros[05] = new SQLiteParameter("@06CDDINAMIC3", "X");
                        parametros[06] = new SQLiteParameter("@07CDGRPDINAM", 71);
                        found = true;
                    }
                    break;
                case "ZCBS":
                    ...
                    ///  ALTERAR NUMEROS
                    else if (NMSEQTAB == 99 || NMSEQTAB == 99)
                    {
                        parametros[03] = new SQLiteParameter("@04CDDINAMIC1", tmpVariaveis.CDORIGPRD);
                        parametros[04] = new SQLiteParameter("@05CDDINAMIC2", "X");
                        parametros[05] = new SQLiteParameter("@06CDDINAMIC3", "X");
                        parametros[06] = new SQLiteParameter("@07CDGRPDINAM", 16);
                        found = true;
                    }
                    ...
                    else if (NMSEQTAB == 12 || NMSEQTAB == 70)
                    {
                        ...
                        if (tmpVariaveis.CDGER0 == "BRG1")
                            ...
                            parametros[03] = new SQLiteParameter("@02CDDINAMIC1", "BR24");
                                ...
                                if (tmpVariaveis.CDGER0 == "BRB3")
                            parametros[03] = new SQLiteParameter("@02CDDINAMIC1", "BR44");
                        else
                    ...
                    case 70:
                        ...
                        if (tmpVariaveis.CDGER0 == "BRG1")
                            ...
                            parametros[04] = new SQLiteParameter("@02CDDINAMIC1", "BR24");
                                ...
                                if (tmpVariaveis.CDGER0 == "BRB3")
                            parametros[04] = new SQLiteParameter("@02CDDINAMIC1", "BR44");
                        else
        ...
        private void ExecutaRegraEPA905()
        {
            ...
            sqlQuery.Append("      ,T1.CDCONDUNID, T1.VLLIMINF, T1.VLLIMSUP, T1.CDUNMED ");
        ...
        private void ExecutaRegraEPA758()
        {
            if (CSEmpresa.ColunaExiste("BRK_EPRECOA758", "CDCONDUNID"))
            {
                sqlQuery.Length = 0;
                sqlQuery.Append("SELECT VLCONDTYP / 1000.000, ");
                sqlQuery.Append("CDCONDUNID, VLLIMINF, VLLIMSUP, CDUNMED ");
                sqlQuery.Append("  FROM BRK_EPRECOA758 ");
                sqlQuery.Append(" WHERE CDAPLICAC = 'V' AND ");
                sqlQuery.Append("       CDCONDTYP = ? AND ");
                sqlQuery.Append("       CDPAIS = 'BR' AND ");
                sqlQuery.Append("       CDORIGPRD = ? AND ");
                sqlQuery.Append("                  IDSITUACAO = '' AND ");
                sqlQuery.Append("       ? BETWEEN DTVIGINI AND DTVIGFIM ");
                sqlQuery.Append(" ORDER BY NMCONDREC DESC ");

                SQLiteParameter[] parametros = new SQLiteParameter[03];

                parametros[00] = new SQLiteParameter("@01CDCONDTYP", CDCONDTYP);
                parametros[01] = new SQLiteParameter("@02CDORIGPRD", tmpVariaveis.CDORIGPRD);
                parametros[02] = new SQLiteParameter("@03DATA", tmpVariaveis.DATA);
        ...
        private decimal? CalculaFormulaBRL()
        {
            ...
            try
            {
                if (CDCONDTYP_ANT == "IPIP")
                {
                    ...
                    {
                        result = ((VALORRET.HasValue) ? VALORRET.Value : VALOR) * quantidadeInteira;
                    }
                ...
                else
                {
                    ...
                    //764
                    if (CDCONDTYP_ANT == "YX23" || CDCONDTYP_ANT == "YX41" || CDCONDTYP_ANT == "YYTA" || NMCONDLIN_ANT == 769 || NMCONDLIN_ANT == 780) //776
        ...
        private decimal? CalculaFormulaF164()
        {
            ...
            try
            {
                ...
                //result = (GetDadoTmpPricingCons(240) == 0) ? 0m : formulaVal * -1m;	 // 240 = DIPI			
                result = (GetSumTmpPricingCons(240, 240) == 0) ? 0m : formulaVal * -1m;	 // 240 = DIPI			
            }
            catch
            {
                result = null;
        ...
        private decimal? CalculaFormulaF320()
        {
            ...
            try
            {
                ...
                if (CDCONDTYP_ANT == "BX40")
                {
                    ...
                    {
                        var a = GetDadoTmpPricingCons(281) != 0m;
                ...
                {
                    if (VALOR == 100 &&
                        GetDadoTmpPricingCons(210) != 0) //210 = ICZF
                    {
                        val = GetSumTmpPricingCons(733, 733); // 708 = BX13
                ...
                if (CDCONDTYP_ANT == "ICMI")
                {
                    ...
                    // 240 = DIPI; 247 = IPXC; 241 = IPVA
                    if ((tmpVariaveis.IDCLIIMP != "C" && tmpVariaveis.IDCLIIMP != "W") ||
                        ((tmpVariaveis.IDCLIIMP == "C" || tmpVariaveis.IDCLIIMP == "W") && (GetDadoTmpPricingCons(240) == 0 || GetDadoTmpPricingCons(247) == 0)) ||
                        (GetDadoTmpPricingCons(241) == 0 && GetDadoTmpPricingCons(248) == 0))
                    {
                        tipoCalculo = 1;
                    }
                    else
                    {
                        if (GetDadoTmpPricingCons(241) == 0) // IPVA
                        {
                            tipoCalculo = 2;
                        }
                        else
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // OBSERVACAO: 08/11/2013 - Caso entre aqui e o valor nao feche com o SAP, ver se alterando a formula conforme abaixo ira resolver, PARTE ONDE ESTA FAZENDO A SUBTRACAO (- &ZPIP) 
                            // Round(&L140 / (( 100 - ( &ICVA * ( &ICBS / 100 ))) / 100 ) + (( &ZPIP / (( 100 - ( &ICVA * ( &ICBS / 100 )))/100)) - &ZPIP),2) 
                ...
                if (CDCONDTYP_ANT == "IBRX")
                {
                    ...
                    val = GetSumTmpPricingCons(699, 699); // 699 = YPIO
                    formulaVal1 = ((val.HasValue) ? val.Value : 0m);
        ...
        private decimal? CalculaFormulaF905()
        {
            ...
            try
            {
                ...
                decimal? val = GetSumTmpPricingCons(783, 783); //780
                val = (val.HasValue) ? val.Value : 0m;

                if (val == 0)
                    val = GetSumTmpPricingCons(784, 784);

                decimal formulaVal1 = (val.HasValue) ? val.Value : 0m;

                //val = GetSumTmpPricingCons(768, 768);
                val = GetSumTmpPricingCons(782, 782); //778
        ...
        private decimal? CalculaFormulaF907()
        {
            ...
            try
            {
                indexVetor = RetornaLocalizacaoNMCONDLIN(283);

                val = GetDadoTmpPricingCons(283); // 283 = ISTM

                valor_ISTM = ((val.HasValue) ? val.Value : 0m);

                if (CDCONDUNID != "BRL")
                    valor_ISTM = valor_ISTM / 100;

                if (tmpPricingCons[indexVetor].CDUNMED == "KGM")
                    valor_ISTM = valor_ISTM * (tmpVariaveis.PESOTOTAL * (quantidadeInteira * quantidadeCaixa + quantidadeFracao));
                else
                    valor_ISTM = valor_ISTM * quantidadeInteira;

                val = GetSumTmpPricingCons(775, 775); // 770 = ISTT
                valor_YSTT = (val.HasValue) ? val.Value : 0m;

                val = GetDadoTmpPricingCons(775); // 770 = ISTT
                valor_ISTT = (val.HasValue) ? val.Value : 0m;
                decimal? ISTN = GetDadoTmpPricingCons(288);

                ISTN = (ISTN.HasValue ? ISTN.Value : 0m) * 100;
                ...
                if (valor_ISTT * 100 == 3)
                {
                        ...
                        ISTN != 100)
                    {
                        ...
                        if (valor_basePadrao >= valor_BaseMinima)
                            result = GetSumTmpPricingCons(777, 777); //772
                        else
                            result = valor_ISTM;
                    }
                    else
                    {
                        val = GetSumTmpPricingCons(777, 777); // 772 = ISTB
