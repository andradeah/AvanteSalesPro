...
namespace AvanteSales
{
    ...
    public class CSEmpregados : CollectionBase
    {
        ...
        public class CSEmpregado
        {
            ...
            public double VLR_TOTAL_VENDA
            {
                get
                {
                    return RETORNAR_TOTAL_VENDA();
                }
            }

            private double RETORNAR_TOTAL_VENDA()
            {
                double totalVenda = 0;

                StringBuilder sqlQuery = new StringBuilder();
                sqlQuery.AppendLine("SELECT SUM(VLR_TOTAL_PEDIDO) FROM PEDIDO ");
                sqlQuery.AppendFormat(" WHERE DATE(DAT_PEDIDO) = DATE('{0}')", DateTime.Now.ToString("yyyy-MM-dd"));
                sqlQuery.AppendFormat(" AND COD_EMPREGADO = {0}", this.COD_EMPREGADO);

                var result = CSDataAccess.Instance.ExecuteScalar(sqlQuery.ToString());

                totalVenda = result == System.DBNull.Value ? 0 : Convert.ToDouble(result);
