...
namespace AvanteSales
{
    public class CSProdutos : CollectionBase
    {
        ...
        public static void AlterarRegistroVencimento(CSProdutos.CSProdutoVencimento produto, int novaQtd, string novoVencimento)
        {
            if (CSEmpresa.ColunaExiste("PDV_PRODUTO_VALIDADE", "COD_PDV"))
            {
                ...
                sql.AppendLine("      ,IND_DESCARREGADO = 0 ");
            ...
            string IND_LIBERA_VENDA_PRODUTO_SEM_ESTOQUE, int COD_TIPO_DISTRIBUICAO_POLITICA, bool ordenaPorCodigo, int COD_DENVER, bool PoliticaFlexx)
        {
            ...
            try
            {
                ...
                sqlQuery.AppendFormat("     ON T5.COD_PRODUTO = T1.COD_PRODUTO AND T5.COD_CATEGORIA = {0} ", (CSEmpresa.Current.IND_CLUSTER_PRODUTOS_PELO_TIPO_FREEZER ? COD_DENVER : COD_CATEGORIA));
                ...
                sqlQuery.AppendFormat("    AND T1.COD_GRUPO = {0} ",COD_GRUPO);
                sqlQuery.AppendFormat("    AND {0} IN (-1, T1.COD_FAMILIA_PRODUTO) ",COD_GRUPO_FAMILIA);
                sqlQuery.AppendFormat("    AND {0} IN (-1, T1.COD_GRUPO_COMERCIALIZACAO) ",COD_GRUPO_COMERCIALIZACAO);
 ...
 : Java.Lang.Object
#endif
        {
            ...
            public decimal RetornarDescontoEscalonado(int quantidadeVenda, int codEscalonamento, int codTabelaPreco)
            {
                decimal desconto = -1;

                StringBuilder sql = new StringBuilder();

                sql.AppendFormat("SELECT COUNT(*) FROM TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO WHERE COD_TABELA_PRECO = {0}", codTabelaPreco);
                ...
                if (Convert.ToInt32(result) > 0)
                {
                    ...
                    sql.AppendLine("SELECT PCT_MAXIMO_DESCONTO ");
                    sql.AppendLine("    FROM ESCALONADA_BLOQUEIO_DESCONTO ");
                    sql.AppendFormat("  WHERE     COD_EMPREGADO = {0} ", codEscalonamento);
                    sql.AppendFormat("        AND COD_PRODUTO = {0} ", this.COD_PRODUTO);
                    sql.AppendFormat("        AND {0} BETWEEN QTD_INICIAL_ESCALONADA AND QTD_INICIAL_ESCALONADA ", quantidadeVenda);

                    using (SqliteDataReader reader = CSDataAccess.Instance.ExecuteReader(sql.ToString()))
                    {
                        if (reader.Read())
                            desconto = reader.GetValue(0) == System.DBNull.Value ? -1 : Convert.ToDecimal(reader.GetValue(0));
                    }
                    ...
                    if (desconto == -1)
                    {
                        ...
                        sql.AppendLine("SELECT PCT_MAXIMO_DESCONTO ");
                        sql.AppendLine("FROM ESCALONADA_BLOQUEIO_DESCONTO ");
                        sql.AppendFormat("WHERE     COD_FAMILIA_PRODUTO = {0} ", this.FAMILIA_PRODUTO.COD_FAMILIA_PRODUTO == 0 ? this.FAMILIA_PRODUTO.COD_FAMILIA_PRODUTO_FILTRADO : this.FAMILIA_PRODUTO.COD_FAMILIA_PRODUTO);
                        sql.AppendFormat("      AND COD_GRUPO = {0} ", this.GRUPO.COD_GRUPO == 0 ? this.GRUPO.COD_GRUPO_FILTRADO : this.GRUPO.COD_GRUPO);
                        sql.AppendFormat("      AND COD_EMPREGADO = {0} ", codEscalonamento);
                        sql.AppendLine("ORDER BY QTD_FINAL_ESCALONADA DESC LIMIT 1 ");
                    ...
                    if (desconto == -1)
                    {
                        ...
                        sql.AppendLine("SELECT PCT_MAXIMO_DESCONTO ");
                        sql.AppendLine("FROM ESCALONADA_BLOQUEIO_DESCONTO ");
                        sql.AppendLine("WHERE       COD_FAMILIA_PRODUTO IS NULL ");
                        sql.AppendFormat("      AND COD_GRUPO = {0} ", this.GRUPO.COD_GRUPO == 0 ? this.GRUPO.COD_GRUPO_FILTRADO : this.GRUPO.COD_GRUPO);
                        sql.AppendFormat("      AND COD_EMPREGADO = {0} ", codEscalonamento);
                        sql.AppendLine("ORDER BY QTD_FINAL_ESCALONADA DESC LIMIT 1 ");
        ...
        public class CSProdutoVencimento : Java.Lang.Object
        {
            ...
            public void AdicionarVencimento()
            {
                ...
                sql.AppendLine("          ,QTD_AVENCER ");
                sql.AppendLine("          ,DAT_VENCIMENTO ");
                sql.AppendLine("          ,IND_DESCARREGADO) ");
