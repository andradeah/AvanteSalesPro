...
namespace AvanteSales
{
    public class CSGruposComercializacao : CollectionBase
    {
        ...
        public class CSGrupoComercializacaoMetaVenda : Java.Lang.Object
        {
            ...
            public decimal PCT_VENDA { get; set; }
            public List<CSGruposComercializacao.CSGrupoComercializacaoMetaVenda> RetornarMetaVendaGrupoComercializacao()
            {
                ...
                sql.AppendLine("      ,(VENDA.TOTAL * 100.0) / OBJ.VLR_OBJETIVO ");
                ...
                sql.AppendLine("SELECT PRODUTO.COD_GRUPO_COMERCIALIZACAO ");
                sql.AppendLine("      ,GC.DES_GRUPO_COMERCIALIZACAO ");
                sql.AppendLine("      ,NULL ");
                sql.AppendLine("      ,SUM(ITEM_PEDIDO.VLR_TOTAL) AS 'TOTAL' ");
                sql.AppendLine("      ,NULL ");
                sql.AppendLine("FROM PEDIDO ");
                sql.AppendLine("JOIN ITEM_PEDIDO ");
                sql.AppendLine("      ON PEDIDO.COD_PEDIDO = ITEM_PEDIDO.COD_PEDIDO ");
                ...
                sql.AppendLine("JOIN PRODUTO ");
                sql.AppendLine("      ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sql.AppendLine("      JOIN GRUPO_COMERCIALIZACAO GC ON  GC.COD_GRUPO_COMERCIALIZACAO = PRODUTO.COD_GRUPO_COMERCIALIZACAO ");
                sql.AppendFormat("WHERE PEDIDO.COD_PDV = {0} AND GC.COD_GRUPO_COMERCIALIZACAO NOT IN(SELECT COD_GRUPO_COMERCIALIZACAO FROM PDV_OBJETIVO WHERE COD_PDV = {0}) ", CSPDVs.Current.COD_PDV);
                sql.AppendLine("GROUP BY PRODUTO.COD_GRUPO_COMERCIALIZACAO, GC.DES_GRUPO_COMERCIALIZACAO ");
                ...
                using (SQLiteDataReader reader = CSDataAccess.Instance.ExecuteReader(sql.ToString()))
                {
                    while (reader.Read())
                    {
                        ...
                        linha.VLR_OBJETIVO = reader.GetValue(2) == System.DBNull.Value ? -1 : reader.GetDouble(2);
                        linha.VLR_VENDIDO = reader.GetValue(3) == System.DBNull.Value ? -1 : reader.GetDouble(3);
                        linha.PCT_VENDA = reader.GetValue(4) == System.DBNull.Value ? -1 : Convert.ToDecimal(reader.GetValue(4));
            ...
            public List<CSGruposComercializacao.CSGrupoComercializacaoMetaVenda> RetornarMetaVendaGrupoComercializacaoVendedor()
            {
                List<CSGruposComercializacao.CSGrupoComercializacaoMetaVenda> resultado = new List<CSGrupoComercializacaoMetaVenda>();
                ...
                sql.AppendLine("         JOIN PRODUTO ");
                sql.AppendLine("             ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sql.AppendFormat("       WHERE PEDIDO.COD_EMPREGADO = {0} ", CSEmpregados.Current.COD_EMPREGADO);
                sql.AppendLine("         GROUP BY PRODUTO.COD_GRUPO_COMERCIALIZACAO ");
                sql.AppendLine("     ) AS VENDA ");
                sql.AppendLine(" ON GC.COD_GRUPO_COMERCIALIZACAO = VENDA.COD_GRUPO_COMERCIALIZACAO ");
                sql.AppendFormat(" WHERE DATE(OBJ.DAT_OBJETIVO) = DATE('{0}') ", DateTime.Now.ToString("yyyy-MM-dd"));
                ...
                sql.AppendLine("      ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sql.AppendLine("      JOIN GRUPO_COMERCIALIZACAO GC ON  GC.COD_GRUPO_COMERCIALIZACAO = PRODUTO.COD_GRUPO_COMERCIALIZACAO ");
                sql.AppendFormat("WHERE PEDIDO.COD_EMPREGADO = {0} AND GC.COD_GRUPO_COMERCIALIZACAO NOT IN(SELECT COD_GRUPO_COMERCIALIZACAO FROM PDV_OBJETIVO WHERE COD_EMPREGADO = {0}) ", CSEmpregados.Current.COD_EMPREGADO);
                sql.AppendLine("GROUP BY PRODUTO.COD_GRUPO_COMERCIALIZACAO, GC.DES_GRUPO_COMERCIALIZACAO ");
