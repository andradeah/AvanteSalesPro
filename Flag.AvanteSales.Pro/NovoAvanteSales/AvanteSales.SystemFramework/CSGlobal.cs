...
namespace AvanteSales
{
    public class CSGlobal
    {
        ...
        private static bool m_ROTINA_VALIDACAO_PEDIDO;
        private static int m_CODIGO_VALIDACAO_PEDIDO;
        private static int m_PDV_VALIDACAO_PEDIDO;
        public static bool ROTINA_VALIDACAO_PEDIDO
        {
            get
            {
                return m_ROTINA_VALIDACAO_PEDIDO;
            }
            set
            {
                m_ROTINA_VALIDACAO_PEDIDO = value;
            }
        }

        public static int CODIGO_VALIDACAO_PEDIDO
        {
            get
            {
                return m_CODIGO_VALIDACAO_PEDIDO;
            }
            set
            {
                m_CODIGO_VALIDACAO_PEDIDO = value;
            }
        }
        public static int PDV_VALIDACAO_PEDIDO
        {
            get
            {
                return m_PDV_VALIDACAO_PEDIDO;
            }
            set
            {
                m_PDV_VALIDACAO_PEDIDO = value;
            }
        }
            ...
            try
            {
                ...
                if (!CSDataAccess.Instance.TableExists("TMP_PEDIDO_EXCLUIDO"))
                {
                    CSDataAccess.Instance.CreateTable("TMP_PEDIDO_EXCLUIDO",
                        ...
                        " ,IND_VLR_DESCONTO_ATUSALDO BIT, IND_VLR_INDENIZACAO_ATUSALDO BIT, MENSAGEM_PEDIDO NVARCHAR(200), RECADO_PEDIDO NVARCHAR(200), IND_FOB BIT, COD_POLITICA_CALCULO_PRECO INT,IND_DESCARREGADO BIT) ");
                }
                ...
                if (CSDataAccess.Instance.TableExists("ESCALONADA_BLOQUEIO_DESCONTO"))
                {
                    if (!CSDataAccess.Instance.TableExists("TMP_BLOQUEIO_DESCONTO_ESCALONADO"))
                    {
                        StringBuilder sqlTemp = new StringBuilder();
                        sqlTemp.AppendLine("CREATE TABLE TMP_BLOQUEIO_DESCONTO_ESCALONADO ( ");
                        sqlTemp.AppendLine("COD_PRODUTO INT NOT NULL ");
                        sqlTemp.AppendLine(",DESCRICAO_PRODUTO CHAR(8) NOT NULL ");
                        sqlTemp.AppendLine(",DSC_PRODUTO VARCHAR(50) NOT NULL ");
                        sqlTemp.AppendLine(",PCT_MAXIMO_DESCONTO DECIMAL(6,2) NOT NULL");
                        sqlTemp.AppendLine(",PCT_DESCONTO_APLICADO DECIMAL(6,2) NOT NULL)");

                        CSDataAccess.Instance.CreateTable("TMP_BLOQUEIO_DESCONTO_ESCALONADO", sqlTemp.ToString());
                    }
                }

                if (!CSEmpresa.ColunaExiste("PEDIDO", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PEDIDO ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PEDIDO SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PEDIDO_INDENIZACAO", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PEDIDO_INDENIZACAO ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PEDIDO_INDENIZACAO SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PRODUTO_COLETA_ESTOQUE", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PRODUTO_COLETA_ESTOQUE ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PRODUTO_COLETA_ESTOQUE SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("ENTRADA_CREDITO_PRODUTO", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE ENTRADA_CREDITO_PRODUTO ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE ENTRADA_CREDITO_PRODUTO SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("HISTORICO_MOTIVO", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE HISTORICO_MOTIVO ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE HISTORICO_MOTIVO SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("MONITORAMENTO_VENDEDOR_ROTA", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE MONITORAMENTO_VENDEDOR_ROTA ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE MONITORAMENTO_VENDEDOR_ROTA SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("RESPOSTA_PESQUISA_MERCADO", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE RESPOSTA_PESQUISA_MERCADO ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE RESPOSTA_PESQUISA_MERCADO SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("RESPOSTA_PESQUISA", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE RESPOSTA_PESQUISA ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE RESPOSTA_PESQUISA SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("MOTIVO_NAORESP_PESQ", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE MOTIVO_NAORESP_PESQ ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE MOTIVO_NAORESP_PESQ SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PESQUISA_MERCHAN_PDV", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PESQUISA_MERCHAN_PDV ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PESQUISA_MERCHAN_PDV SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PDV_EMAIL", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PDV_EMAIL ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PDV_EMAIL SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("MOTIVO_NAO_PESQUISA", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE MOTIVO_NAO_PESQUISA ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE MOTIVO_NAO_PESQUISA SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PDV_VISITA", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PDV_VISITA ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PDV_VISITA SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("PDV_PRODUTO_VALIDADE", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE PDV_PRODUTO_VALIDADE ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PDV_VISITA SET IND_DESCARREGADO = 1");
                }

                if (!CSEmpresa.ColunaExiste("GRUPO_MARKUP", "IND_DESCARREGADO"))
                {
                    CSDataAccess.Instance.ExecuteNonQuery("ALTER TABLE GRUPO_MARKUP ADD IND_DESCARREGADO BIT");
                    CSDataAccess.Instance.ExecuteNonQuery("UPDATE PDV_VISITA SET IND_DESCARREGADO = 1");
                }
            }
            catch (Exception ex)
