    ...
    public class CSDescarga
    {
        ...
        public static DataSet MontaDataSetDescarga(bool Descarga,string versaoAvante)
        {
            ...
            try
            {
                ...
                sqlPedido.AppendLine(" AND BOL_PEDIDO_VALIDADO = 1 ");
                ...
                sqlPedido.AppendLine(" AND IND_DESCARREGADO = 0 ");

                sqlPedido.AppendLine(" ORDER BY COD_PEDIDO DESC");

                dtPedido = CSDataAccess.Instance.ExecuteDataTable(sqlPedido.ToString());
#else
                ...
                sqlItemPedido.AppendLine(" AND T2.BOL_PEDIDO_VALIDADO = 1 ");

                if (CSEmpresa.ColunaExiste("PEDIDO", "IND_PEDIDO_RETORNADO"))
                    sqlItemPedido.AppendLine(" AND (T2.IND_PEDIDO_RETORNADO = 0 OR T2.IND_PEDIDO_RETORNADO IS NULL) ");

                sqlItemPedido.AppendLine(" AND T2.IND_DESCARREGADO = 0 ");

                sqlItemPedido.AppendLine(" ORDER BY T1.COD_PEDIDO DESC ");

                dtItemPedido = CSDataAccess.Instance.ExecuteDataTable(sqlItemPedido.ToString());
#else
                ...
                if (CSDataAccess.Instance.TableExists("PRODUTO_COLETA_ESTOQUE"))
                {
                    ...
                    dtProdutoColetaEstoque = CSDataAccess.Instance.ExecuteDataTable(
                        ...
                        " WHERE T1.IND_DESCARREGADO = 0  " +
                        "   AND T1.QTD_COLETADA >= 0");
                ...
                sqlHistoricoMotivo.Append("WHERE IND_DESCARREGADO = 0  AND (IND_HISTORICO <> 1 OR IND_HISTORICO IS NULL) ");
                ...
                dtHistoricoMotivo = CSDataAccess.Instance.ExecuteDataTable(sqlHistoricoMotivo.ToString());
                //"SELECT DISTINCT * " +
                ...
                if (CSDataAccess.Instance.TableExists("PDV_EMAIL"))
                {
                    dtPdv_email = CSDataAccess.Instance.ExecuteDataTable(
                        string.Format("SELECT * FROM PDV_EMAIL WHERE IND_ALTERADO = 1  AND IND_DESCARREGADO = 0 "));
                ...
                if (CSDataAccess.Instance.TableExists("MOTIVO_NAO_PESQUISA"))
                {
                    dtMotivoNaoPesquisaMercado = CSDataAccess.Instance.ExecuteDataTable(
                        string.Format("SELECT * FROM MOTIVO_NAO_PESQUISA WHERE IND_DESCARREGADO = 0 "));
                ...
                if (CSDataAccess.Instance.TableExists("PDV_VISITA"))
                {
                    ...
                    if (!Descarga)
                        sql.AppendFormat("WHERE IND_DESCARREGADO = 0 ");
