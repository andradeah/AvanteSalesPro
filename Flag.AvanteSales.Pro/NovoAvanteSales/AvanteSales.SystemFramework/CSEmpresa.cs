...
namespace AvanteSales
{
    public class CSEmpresa
    {
        ...
        private bool m_IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO;
        ...
        public bool IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO
        {
            get
            {
                return m_IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO;
            }
            set
            {
                m_IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO = value;
            }
        }
        ...
        public void ATUALIZA_EMPRESA()
        {
            try
            {
                ...
                if (ColunaExiste("EMPRESA", "IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO"))
                    sqlQuery.Append(", IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO");
                ...
                using (SQLiteDataReader sqlReader = CSDataAccess.Instance.ExecuteReader(sqlQuery.ToString(), CommandBehavior.SingleResult))
                {
                    while (sqlReader.Read())
                    {
                        ...
                        {
                            valor++;
                            this.IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO = sqlReader.GetValue(valor) == System.DBNull.Value ? false :
                                (sqlReader.GetString(valor).ToUpper() == "S" ? true : false);
                        }
                        else
                            this.IND_UTILIZA_PRIORIDADE_TIPO_DOCUMENTO = false;
        ...
        public static List<string> InformacoesAcesso(string query)
        {
            ...
            using (SQLiteDataReader sqlReader = CSDataAccess.Instance.ExecuteReader(query))
            {
                while (sqlReader.Read())
                {
                    ...
                    informacoes.Add(sqlReader.GetValue(1).ToString());
