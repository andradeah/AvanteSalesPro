        ...
        public class CSPDVEmail
        {
            ...
            public void AdicionarEmail(CSPDVEmail novoEmail)
            {
                try
                {
                    ...
                    StringBuilder sql = new StringBuilder();
                    sql.Append("INSERT INTO PDV_EMAIL(IND_ALTERADO,COD_SEQUENCIA,COD_PDV,COD_TIPO_EMAIL,DSC_EMAIL,DAT_ALTERACAO, IND_DESCARREGADO) VALUES ");
                    sql.AppendFormat("(1,{0} ", sequencia);
                    sql.AppendFormat(",{0},{1},'{2}',DATETIME('{3}'),0)", CSPDVs.Current.COD_PDV, novoEmail.COD_TIPO_EMAIL, novoEmail.DSC_EMAIL, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            public void EditarEmail()
            {
                ...
                {
                    StringBuilder sql = new StringBuilder();
                    sql.AppendFormat("UPDATE PDV_EMAIL SET IND_ALTERADO = 1,DSC_EMAIL = '{0}',COD_TIPO_EMAIL = {1},DAT_ALTERACAO = DATETIME('{2}'),IND_DESCARREGADO = 0 ",
                                                                                                                           Current.DSC_EMAIL,
