...
namespace AvanteSales
{
    ...
    public class CSPDVs : CollectionBase, IDisposable
    {
        ...
        public static bool RetornarPdvValidarDescontoEscalonado(int cod_categoria)
        {
            bool retorno = false;

            StringBuilder sql = new StringBuilder();

            if (CSEmpresa.ColunaExiste("ESCALONADA_BLOQUEIO_DESCONTO", "COD_EMPREGADO"))
            {
                sql.Append("SELECT COUNT(*) FROM TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO WHERE COD_CATEGORIA IS NOT NULL");
                ...
                else
                {
                    ...
                    sql.AppendFormat("SELECT COUNT(*) FROM TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO WHERE COD_CATEGORIA = {0}", cod_categoria);

                    result = CSDataAccess.Instance.ExecuteScalar(sql.ToString());
 ...
 : Java.Lang.Object
...

        {
            ...
            private bool m_VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO;
            #endregion
            ...
            public bool VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO
            {
                get
                {
                    return m_VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO;
                }
                set
                {
                    m_VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO = value;
                }
            }
