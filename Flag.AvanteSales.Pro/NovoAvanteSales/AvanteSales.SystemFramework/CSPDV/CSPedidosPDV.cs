...
namespace AvanteSales
{
    public class CSPedidosPDV : CollectionBase, IDisposable
    {
        ...
        public int RETORNAR_EMPREGADO_ESCALONAMENTO()
        {
            int codEmpregado = -1;

            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT DISTINCT T1.COD_EMPREGADO,T2.COD_EMPREGADO,T3.COD_EMPREGADO ");
            sql.AppendLine("    FROM EMPREGADO ");
            sql.AppendLine("    LEFT JOIN ESCALONADA_BLOQUEIO_DESCONTO T1 ");
            sql.AppendLine("        ON T1.COD_EMPREGADO = EMPREGADO.COD_EMPREGADO ");
            sql.AppendLine("    LEFT JOIN ESCALONADA_BLOQUEIO_DESCONTO T2 ");
            sql.AppendLine("        ON T2.COD_EMPREGADO = EMPREGADO.COD_GERENTE ");
            sql.AppendLine("    LEFT JOIN ESCALONADA_BLOQUEIO_DESCONTO T3 ");
            sql.AppendLine("        ON T3.COD_EMPREGADO = EMPREGADO.COD_SUPERVISOR ");
            sql.AppendLine("    WHERE(T1.COD_EMPREGADO IS NOT NULL ");
            sql.AppendLine("          OR T2.COD_EMPREGADO IS NOT NULL ");
            sql.AppendLine("          OR T3.COD_EMPREGADO IS NOT NULL) AND ");
            sql.AppendFormat("           EMPREGADO.COD_EMPREGADO = {0} ", CSEmpregados.Current.COD_EMPREGADO);

            using (SQLiteDataReader sqlReader = CSDataAccess.Instance.ExecuteReader(sql.ToString()))
            {
                if (sqlReader.Read())
                {
                    var empregado = sqlReader.GetValue(0) == System.DBNull.Value ? -1 : sqlReader.GetInt32(0);

                    if (empregado == -1)
                    {
                        var gerente = sqlReader.GetValue(1) == System.DBNull.Value ? -1 : sqlReader.GetInt32(1);

                        if (gerente == -1)
                        {
                            var supervisor = sqlReader.GetValue(2) == System.DBNull.Value ? -1 : sqlReader.GetInt32(2);

                            if (supervisor != -1)
                                codEmpregado = supervisor;
                        }
                        else
                            codEmpregado = gerente;
                    }
                    else
        ...
        public void Flush()
        {
            try
            {
                ...
                sqlQueryInsert += ",IND_DESCARREGADO)";
                ...
                sqlQueryUpdate += ",IND_DESCARREGADO = 0";
                ...
                string sqlQueryInsertTmp =
                   ...
                   "                                ,COD_MOT_INDENIZACAO, IND_VLR_DESCONTO_ATUSALDO, IND_VLR_INDENIZACAO_ATUSALDO " +
                   "                                ,MENSAGEM_PEDIDO, RECADO_PEDIDO, IND_FOB, COD_POLITICA_CALCULO_PRECO,IND_DESCARREGADO) " +
                   ...
                   "        ,COD_PDV_SOLDTO, NUM_DOC_INDENIZACAO, COD_TIPO_MOT_INDENIZACAO " +
                   "        ,COD_MOT_INDENIZACAO, IND_VLR_DESCONTO_ATUSALDO, IND_VLR_INDENIZACAO_ATUSALDO " +
                   "        ,MENSAGEM_PEDIDO, RECADO_PEDIDO, IND_FOB, COD_POLITICA_CALCULO_PRECO,0 " +
                   "    FROM PEDIDO " +
 ...
 IDisposable
        {
            ...
            private int m_COD_EMPREGADO_ESCALONAMENTO;
            #endregion
            ...
            public int COD_EMPREGADO_ESCALONAMENTO
            {
                get
                {
                    return m_COD_EMPREGADO_ESCALONAMENTO;
                }
                set
                {
                    m_COD_EMPREGADO_ESCALONAMENTO = value;
                }
            }
            ...
            public List<string[]> ValidarDescontoEscalonado(int cod_empregado_escalonamento)
            {
                List<string[]> naoValidos = new List<string[]>();

                CSDataAccess.Instance.ExecuteNonQuery("DELETE FROM TMP_BLOQUEIO_DESCONTO_ESCALONADO");

                StringBuilder sqlQuery1 = new StringBuilder();
                sqlQuery1.AppendLine("INSERT INTO TMP_BLOQUEIO_DESCONTO_ESCALONADO ");
                sqlQuery1.AppendLine("SELECT DISTINCT PRODUTO.COD_PRODUTO ");
                sqlQuery1.AppendLine("    ,PRODUTO.DESCRICAO_APELIDO_PRODUTO ");
                sqlQuery1.AppendLine("	  ,PRODUTO.DSC_PRODUTO ");
                sqlQuery1.AppendLine("	  ,ESCALONADA_BLOQUEIO_DESCONTO.PCT_MAXIMO_DESCONTO ");
                sqlQuery1.AppendLine("    ,ITEM_PEDIDO.PRC_DESCONTO ");
                sqlQuery1.AppendLine("FROM ITEM_PEDIDO ");
                sqlQuery1.AppendLine("JOIN PRODUTO ");
                sqlQuery1.AppendLine("    ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sqlQuery1.AppendLine("JOIN ESCALONADA_BLOQUEIO_DESCONTO ");
                sqlQuery1.AppendLine("    ON PRODUTO.COD_GRUPO = ESCALONADA_BLOQUEIO_DESCONTO.COD_GRUPO ");
                sqlQuery1.AppendLine("    AND PRODUTO.COD_FAMILIA_PRODUTO = ESCALONADA_BLOQUEIO_DESCONTO.COD_FAMILIA_PRODUTO ");
                sqlQuery1.AppendFormat("  AND ESCALONADA_BLOQUEIO_DESCONTO.COD_EMPREGADO = {0} ", cod_empregado_escalonamento);
                sqlQuery1.AppendLine("JOIN (SELECT  PRODUTO.COD_GRUPO ");
                sqlQuery1.AppendLine("             ,PRODUTO.COD_FAMILIA_PRODUTO ");
                sqlQuery1.AppendLine("             ,SUM(CAST((ITEM_PEDIDO.QTD_PEDIDA / PRODUTO.QTD_UNIDADE_EMBALAGEM) AS INT)) AS 'QTDVENDIDA' ");
                sqlQuery1.AppendLine("        FROM ITEM_PEDIDO ");
                sqlQuery1.AppendLine("        JOIN PRODUTO ");
                sqlQuery1.AppendLine("            ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sqlQuery1.AppendFormat("        WHERE ITEM_PEDIDO.COD_PEDIDO = {0} ", this.COD_PEDIDO);
                sqlQuery1.AppendLine("      GROUP BY PRODUTO.COD_GRUPO, PRODUTO.COD_FAMILIA_PRODUTO ");
                sqlQuery1.AppendLine("    ) GRUPO_PRODUTO_VENDA ");
                sqlQuery1.AppendLine("    ON  ESCALONADA_BLOQUEIO_DESCONTO.COD_GRUPO = GRUPO_PRODUTO_VENDA.COD_GRUPO ");
                sqlQuery1.AppendLine("    AND ESCALONADA_BLOQUEIO_DESCONTO.COD_FAMILIA_PRODUTO = GRUPO_PRODUTO_VENDA.COD_FAMILIA_PRODUTO ");
                sqlQuery1.AppendLine("JOIN TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO ");
                sqlQuery1.AppendLine("    ON TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO.COD_TABELA_PRECO = ITEM_PEDIDO.COD_TABELA_PRECO ");
                sqlQuery1.AppendLine("WHERE GRUPO_PRODUTO_VENDA.QTDVENDIDA ");
                sqlQuery1.AppendLine("         BETWEEN ESCALONADA_BLOQUEIO_DESCONTO.QTD_INICIAL_ESCALONADA AND ESCALONADA_BLOQUEIO_DESCONTO.QTD_FINAL_ESCALONADA ");
                sqlQuery1.AppendFormat("       AND ITEM_PEDIDO.COD_PEDIDO = {0} ", this.COD_PEDIDO);
                sqlQuery1.AppendLine("         AND ITEM_PEDIDO.PRC_DESCONTO > ESCALONADA_BLOQUEIO_DESCONTO.PCT_MAXIMO_DESCONTO ");

                CSDataAccess.Instance.ExecuteNonQuery(sqlQuery1.ToString());

                StringBuilder sqlQuery2 = new StringBuilder();
                sqlQuery2.AppendLine("INSERT INTO TMP_BLOQUEIO_DESCONTO_ESCALONADO ");
                sqlQuery2.AppendLine("SELECT DISTINCT PRODUTO.COD_PRODUTO ");
                sqlQuery2.AppendLine("    ,PRODUTO.DESCRICAO_APELIDO_PRODUTO ");
                sqlQuery2.AppendLine("	  ,PRODUTO.DSC_PRODUTO ");
                sqlQuery2.AppendLine("	  ,ESCALONADA_BLOQUEIO_DESCONTO.PCT_MAXIMO_DESCONTO ");
                sqlQuery2.AppendLine("    ,ITEM_PEDIDO.PRC_DESCONTO ");
                sqlQuery2.AppendLine("FROM ITEM_PEDIDO ");
                sqlQuery2.AppendLine("JOIN PRODUTO ");
                sqlQuery2.AppendLine("    ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sqlQuery2.AppendLine("JOIN ESCALONADA_BLOQUEIO_DESCONTO ");
                sqlQuery2.AppendLine("    ON PRODUTO.COD_GRUPO = ESCALONADA_BLOQUEIO_DESCONTO.COD_GRUPO ");
                sqlQuery2.AppendLine("    AND ESCALONADA_BLOQUEIO_DESCONTO.COD_FAMILIA_PRODUTO IS NULL ");
                sqlQuery2.AppendFormat("  AND ESCALONADA_BLOQUEIO_DESCONTO.COD_EMPREGADO = {0} ", cod_empregado_escalonamento);
                sqlQuery2.AppendLine("JOIN (SELECT  PRODUTO.COD_GRUPO ");
                sqlQuery2.AppendLine("             ,PRODUTO.COD_FAMILIA_PRODUTO ");
                sqlQuery2.AppendLine("             ,SUM(CAST((ITEM_PEDIDO.QTD_PEDIDA / PRODUTO.QTD_UNIDADE_EMBALAGEM) AS INT)) AS 'QTDVENDIDA' ");
                sqlQuery2.AppendLine("        FROM ITEM_PEDIDO ");
                sqlQuery2.AppendLine("        JOIN PRODUTO ");
                sqlQuery2.AppendLine("            ON ITEM_PEDIDO.COD_PRODUTO = PRODUTO.COD_PRODUTO ");
                sqlQuery2.AppendFormat("       WHERE ITEM_PEDIDO.COD_PEDIDO = {0} ", this.COD_PEDIDO);
                sqlQuery2.AppendLine("            GROUP BY PRODUTO.COD_GRUPO, PRODUTO.COD_FAMILIA_PRODUTO");
                sqlQuery2.AppendLine("    ) GRUPO_PRODUTO_VENDA ");
                sqlQuery2.AppendLine("    ON  ESCALONADA_BLOQUEIO_DESCONTO.COD_GRUPO = GRUPO_PRODUTO_VENDA.COD_GRUPO ");
                sqlQuery2.AppendLine("JOIN TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO ");
                sqlQuery2.AppendLine("    ON TABELA_CATEGORIA_ESCALONADA_BLOQUEIO_DESCONTO.COD_TABELA_PRECO = ITEM_PEDIDO.COD_TABELA_PRECO ");
                sqlQuery2.AppendLine("WHERE GRUPO_PRODUTO_VENDA.QTDVENDIDA ");
                sqlQuery2.AppendLine("         BETWEEN ESCALONADA_BLOQUEIO_DESCONTO.QTD_INICIAL_ESCALONADA AND ESCALONADA_BLOQUEIO_DESCONTO.QTD_FINAL_ESCALONADA ");
                sqlQuery2.AppendFormat("    AND ITEM_PEDIDO.COD_PEDIDO = {0} ", this.COD_PEDIDO);
                sqlQuery2.AppendLine("      AND ITEM_PEDIDO.PRC_DESCONTO > ESCALONADA_BLOQUEIO_DESCONTO.PCT_MAXIMO_DESCONTO ");

                CSDataAccess.Instance.ExecuteNonQuery(sqlQuery2.ToString());

                StringBuilder sqlProdutosNaoValidos = new StringBuilder();
                sqlProdutosNaoValidos.Append("SELECT * FROM TMP_BLOQUEIO_DESCONTO_ESCALONADO");

                using (SqliteDataReader reader = CSDataAccess.Instance.ExecuteReader(sqlProdutosNaoValidos.ToString()))
                {
                    while (reader.Read())
                    {
                        string[] registro = new string[5];

                        registro[0] = reader.GetValue(0) == System.DBNull.Value ? string.Empty : reader.GetInt32(0).ToString();
                        registro[1] = registro.GetValue(1) == System.DBNull.Value ? string.Empty : reader.GetString(1);
                        registro[2] = reader.GetValue(2) == System.DBNull.Value ? string.Empty : reader.GetString(2);
                        registro[3] = reader.GetValue(3) == System.DBNull.Value ? string.Empty : reader.GetDecimal(3).ToString(CSGlobal.DecimalStringFormat) + "%";
                        registro[4] = reader.GetValue(4) == System.DBNull.Value ? string.Empty : reader.GetDecimal(4).ToString(CSGlobal.DecimalStringFormat) + "%";
                        
                        naoValidos.Add(registro);
                    }
                }
            ...
            public void DELETAR_ITEM(int cod_produto)
            {
                StringBuilder sql = new StringBuilder();
                sql.AppendFormat("DELETE FROM ITEM_PEDIDO WHERE COD_PEDIDO = {0} AND COD_PRODUTO = {1}", this.COD_PEDIDO, cod_produto);
