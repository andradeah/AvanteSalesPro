...
namespace AvanteSales
{
    public class CSHistoricosMotivo : CollectionBase
    {
        ...
        public bool Flush()
        {
            ...
            sqlQueryInsert.AppendLine(" ,IND_DESCARREGADO)");
            sqlQueryInsert.AppendLine("VALUES(?,?,?,?,?,?,?");
            sqlQueryInsert.AppendLine(CSEmpresa.ColunaExiste("HISTORICO_MOTIVO", "NUM_CPF_EMPREGADO") ? ",?" : "");

            sqlQueryInsert.AppendLine(",0)");

            string sqlQueryUpdate =
                 "UPDATE HISTORICO_MOTIVO SET COD_MOTIVO=?, DAT_HISTORICO_MOTIVO=?, IND_DESCARREGADO = 0 WHERE COD_PDV=? AND date(DAT_HISTORICO_MOTIVO)=date(?) AND COD_TIPO_MOTIVO=?";
