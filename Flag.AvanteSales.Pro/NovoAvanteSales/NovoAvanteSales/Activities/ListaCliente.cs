...
namespace AvanteSales.Pro.Activities
{
    ...
    public class ListaCliente : AppCompatActivity
    {
        ...
        TextView lblValorMeta;
        TextView lblValorReal;
        ...
        static LinearLayout llGrafico;
        static LinearLayout llGraficoAnimacao;
        static TextView lblPorcentagemVenda;
        ...
        {
            if (CSEmpregados.Current.VLR_META_EMPREGADO != 0 &&
                CSEmpregados.Current.VLR_TOTAL_VENDA != 0)
                lblPorcentagemVenda.Text = string.Format("{0} %", ((CSEmpregados.Current.VLR_TOTAL_VENDA * 100) / CSEmpregados.Current.VLR_META_EMPREGADO).ToString(".0"));
            ...
            try
            {
                ...
                if (indexPDV != -1)
                {
                    ...
                    pdv.VLR_TOTAL_VENDAS = vlr_venda;
                    ...
                    Button btnMeta = linhaASerAtualizada.FindViewById<Button>(Resource.Id.btnMeta);
                    LinearLayout llStatusPositivacao = linhaASerAtualizada.FindViewById<LinearLayout>(Resource.Id.llStatusPositivacao);

                    lblValorReal.Text = string.Format("R$ {0}", CSEmpregados.Current.VLR_TOTAL_VENDA == 0 ? "-" : string.Format("{0:0,0.00}", CSEmpregados.Current.VLR_TOTAL_VENDA));

                    if (pdv.VLR_META_PDV == 0)
                    {
                        btnMeta.SetCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);

                        if (pdv.VLR_TOTAL_VENDAS == 0)
                            btnMeta.Text = string.Format("- De -");
                        else
                            btnMeta.Text = string.Format("R$ {0} De -", string.Format("{0:0,0.00}", vlr_venda));
                    }
                    else
                    {
                        if (vlr_venda == 0)
                        {
                            btnMeta.Text = string.Format("- De R$ {0}", string.Format("{0:0,0.00}", pdv.VLR_META_PDV));

                            btnMeta.SetCompoundDrawablesWithIntrinsicBounds(Resource.Drawable.alvo_vermelho, 0, 0, 0);
                        }
                        else
                        {
                            btnMeta.Text = string.Format("R$ {0} De R$ {1}", string.Format("{0:0,0.00}", vlr_venda), string.Format("{0:0,0.00}", pdv.VLR_META_PDV));

                            int porcentagemVenda = Convert.ToInt32((vlr_venda * 100) / pdv.VLR_META_PDV);

                            if (porcentagemVenda < 50)
                                btnMeta.SetCompoundDrawablesWithIntrinsicBounds(Resource.Drawable.alvo_vermelho, 0, 0, 0);
                            else if (porcentagemVenda < 100)
                                btnMeta.SetCompoundDrawablesWithIntrinsicBounds(Resource.Drawable.alvo_laranja, 0, 0, 0);
                            else
                                btnMeta.SetCompoundDrawablesWithIntrinsicBounds(Resource.Drawable.alvo_verde, 0, 0, 0);
                        }
                    }

                    if (pdv.IND_POSITIVADO)
                    {
                        llStatusPositivacao.SetBackgroundColor(Color.ParseColor("#03a9f4"));
                    }
                    else
                    {
                        ...
                        {
                            llStatusPositivacao.SetBackgroundColor(Color.ParseColor("#FF001B"));
                        }
                    }
                }
                    ...
                    CSEmpregados.Current.VLR_TOTAL_VENDA != 0)
                {
                    ...
                    new ThreadCarregaGrafico().Execute();
                }
            }
        ...
        private void Inicializacao()
        {
            ...
            lblValorMeta.Text = string.Format("R$ {0}", CSEmpregados.Current.VLR_META_EMPREGADO == 0 ? "-" : string.Format("{0:0,0.00}", CSEmpregados.Current.VLR_META_EMPREGADO));
            lblValorReal.Text = string.Format("R$ {0}", CSEmpregados.Current.VLR_TOTAL_VENDA == 0 ? "-" : string.Format("{0:0,0.00}", CSEmpregados.Current.VLR_TOTAL_VENDA));
        ...
        private void Eventos()
        {
            ...
            llGrafico.Click += LlGrafico_Click;
        }

        private void LlGrafico_Click(object sender, EventArgs e)
        {
            Android.Support.V4.App.DialogFragment df = new DialogMetaLinha();

            Bundle arguments = new Bundle();
            arguments.PutBoolean("ResumoPorEmpregado", true);
        ...
        private void FindViewsById()
        {
            ...
            lblValorMeta = FindViewById<TextView>(Resource.Id.lblValorMeta);
            lblValorReal = FindViewById<TextView>(Resource.Id.lblValorReal);
            llGrafico = FindViewById<LinearLayout>(Resource.Id.llGrafico);
            llGraficoAnimacao = FindViewById<LinearLayout>(Resource.Id.llGraficoAnimacao);
            lblPorcentagemVenda = FindViewById<TextView>(Resource.Id.lblPorcentagemVenda);
        }
        ...
        private class ThreadCarregarPdv : AsyncTask
        {
            ...
            protected override void OnPostExecute(Java.Lang.Object result)
            {
                ...
                if (CSGlobal.ROTINA_VALIDACAO_PEDIDO)
                {
                    var pdv = pdvsConvertidos.Cast<CSPDVs.CSPDV>().Where(p => p.COD_PDV == CSGlobal.PDV_VALIDACAO_PEDIDO).FirstOrDefault();
        ...
        private class ThreadCarregaGrafico : AsyncTask
        {
            protected override Java.Lang.Object DoInBackground(params Java.Lang.Object[] @params)
            {
                return true;
            ...
            protected override void OnPostExecute(Java.Lang.Object result)
            {
                ...
                PreencherGrafico(CSEmpregados.Current.VLR_META_EMPREGADO, CSEmpregados.Current.VLR_TOTAL_VENDA);
            }
        }
        ...
        private static void PreencherGrafico(double vlrMeta, double vlrTotalVenda)
        {
            if (vlrTotalVenda != 0 &&
                vlrMeta != 0)
            {
                llGrafico.RemoveAllViews();

                int widthLayout = llGrafico.Width;

                double porcentagemVenda = (vlrTotalVenda * 100) / vlrMeta;

                int widthPorcentagemVenda;

                if (porcentagemVenda >= 100)
                    widthPorcentagemVenda = widthLayout;
                else
                    widthPorcentagemVenda = Convert.ToInt32(System.Math.Round(widthLayout * (porcentagemVenda / 100), MidpointRounding.AwayFromZero));

                LinearLayout llteste = new LinearLayout(CurrentActivity);
                llteste.LayoutParameters = new ViewGroup.LayoutParams(0, llGrafico.Height / 4);
                llteste.SetBackgroundColor(Color.ParseColor("#F5841F"));

                llGrafico.AddView(llteste);

                var animacao = new ResizeAnimation(llteste, 0, llGrafico.Height / 4, widthPorcentagemVenda, llGrafico.Height / 4);

                llteste.StartAnimation(animacao);
            }
            else
                llGrafico.Visibility = ViewStates.Gone;
        }
        ...
        class LitemItemAdapter : ArrayAdapter<CSPDVs.CSPDV>
        {
            ...
            private LinearLayout llStatusPositivacao;
            ...
            public override View GetView(int position, View convertView, ViewGroup parent)
            {
                ...
                if (pdv != null)
                {
                    ...
                    llStatusPositivacao = convertView.FindViewById<LinearLayout>(Resource.Id.llStatusPositivacao);
                    ...
                    if (RazaoSocial != null)
                    {
                        ...
                        if (pdv.VLR_META_PDV == 0)
                        {
                            ...
                            if (pdv.VLR_TOTAL_VENDAS == 0)
                                ...
                                btnMeta.Text = string.Format("R$ {0} De -", string.Format("{0:0,0.00}", pdv.VLR_TOTAL_VENDAS));
                        }
                        else
                        {
                            if (pdv.VLR_TOTAL_VENDAS == 0)
                            ...
                            {
                                int porcentagemVenda = Convert.ToInt32((pdv.VLR_TOTAL_VENDAS * 100) / pdv.VLR_META_PDV);
                                ...
                                btnMeta.Text = string.Format("R$ {0} De R$ {1}", string.Format("{0:0,0.00}", pdv.VLR_TOTAL_VENDAS), string.Format("{0:0,0.00}", pdv.VLR_META_PDV));
                            }
                        }
                        ...
                        {
                            llStatusPositivacao.SetBackgroundColor(Color.ParseColor("#7f7f7f"));
                        }
                    }
                }
                return convertView;
            ...
            private void BtnTotalVendas_Click(object sender, EventArgs e)
            {
                ...
                if (((CSPDVs.CSPDV)listPdvs.Adapter.GetItem(position)).VLR_META_PDV != 0)
                {
                    ...
                    Bundle arguments = new Bundle();
                    arguments.PutBoolean("ResumoPorEmpregado", false);
                    df.Arguments = arguments;
            ...
            private void BtnMeta_Click(object sender, EventArgs e)
            {
                ...
                if (((CSPDVs.CSPDV)listPdvs.Adapter.GetItem(position)).VLR_META_PDV != 0 ||
                    ((CSPDVs.CSPDV)listPdvs.Adapter.GetItem(position)).IND_POSITIVADO)
                {
                    if (MetaVendaClick)
                        return;
