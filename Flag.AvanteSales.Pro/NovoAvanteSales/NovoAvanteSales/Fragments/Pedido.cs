    ...
    public class Pedido : Android.Support.V4.App.Fragment
    {
        ...
        private static bool ValidaDados3()
        {
            try
            {
                ...
                if (!ValidaBloqueioDescontoEscalonado())
                    return false;
        ...
        private static bool ValidaBloqueioDescontoEscalonado()
        {
            if (!CSPDVs.Current.VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO)
                return true;

            List<string[]> produtosNaoValidos = CSPDVs.Current.PEDIDOS_PDV.Current.ValidarDescontoEscalonado(CSPDVs.Current.PEDIDOS_PDV.Current.COD_EMPREGADO_ESCALONAMENTO);

            if (produtosNaoValidos.Count > 0)
            {
                ((Cliente)ActivityContext).AbrirDialogFragmentProdutosNaoValidosEscalonamento(produtosNaoValidos);
        ...
        public override void OnViewCreated(View view, Bundle savedInstanceState)
        {
            try
            {
                ...
                if (CSPDVs.Current.VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO &&
                    CSPDVs.Current.PEDIDOS_PDV.Current.COD_EMPREGADO_ESCALONAMENTO == 0)
                    CSPDVs.Current.PEDIDOS_PDV.Current.COD_EMPREGADO_ESCALONAMENTO = CSPDVs.Current.PEDIDOS_PDV.RETORNAR_EMPREGADO_ESCALONAMENTO();
        ...
        private class ThreadMotivoIndicados : AsyncTask
        {
            ...
            protected override void OnPostExecute(Java.Lang.Object result)
            {
                if (Convert.ToInt32(result) == 2)
                {
                    ...
                    ((Cliente)ActivityContext).FragmentCarregando = false;
                }
                else
                    new ThreadInicializacao().Execute();
            }
            ...
            private void CarregaComboBoxCondicaoPagamento()
            {
                try
                {
                    ...
                    for (int i = 0; i < CSCondicoesPagamento.Items.Count; i++)
                    {
                        ...
                        {
                            if (CSPDVs.Current.PEDIDOS_PDV.Current.OPERACAO.COD_CONDICAO_PAGAMENTO != 0)
                            {
                                if (condpag.COD_CONDICAO_PAGAMENTO != CSPDVs.Current.PEDIDOS_PDV.Current.OPERACAO.COD_CONDICAO_PAGAMENTO)
                                    continue;
                            }
