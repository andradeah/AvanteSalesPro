...
namespace AvanteSales.Pro.Fragments
{
    public class ProdutoVenda : Android.Support.V4.App.Fragment, TextView.IOnEditorActionListener
    {
        ...
        private static bool ValidaQuantidadeAbatimento()
        {
            try
            {
                if (ProdutoAbatimento.txtQtdeInteiroIndenizacao != null &&
                    GetQtdPedidaUnidadeIndenizacao() > GetQtdPedidaUnidade() && CSEmpresa.Current.IND_INDENIZACAO_MAIORVENDA == false)
                {
            ...
            try
            {
                if (CSGlobal.StrToDecimal(txtQtdeInteiro.Text) != 0 || CSGlobal.StrToInt(txtQtdeUnidade.Text) != 0)
                {
                    ...
                    if (CSGlobal.StrToInt(txtQtdeUnidade.Text) > 0)
                    {
                        ...
                        {
                            MessageBox.ShowShortMessageCenter(ActivityContext, "Este produto só pode ser vendido inteiro.");
                            txtQtdeUnidade.RequestFocus();
                            return false;
                        }
                    }
        ...
        private static bool ValidaDados()
        {
            try
            {
                ...
                if (ProdutoAbatimento.txtQtdeInteiroIndenizacao != null &&
                    !ValidaQuantidadeUnidadeIndenizacao())
                    return false;
        ...
        private static bool DescontoBloqueioEscalonado()
        {
            try
            {
                if (CSPDVs.Current.VALIDAR_BLOQUEIO_DESCONTO_ESCALONADO &&
                    CSGlobal.StrToDecimal(txtQtdeInteiro.Text) > 0 &&
                    CSGlobal.StrToDecimal(txtDescIncond.Text) > 0)
                {
                    decimal descontoMaximo = CSProdutos.Current.RetornarDescontoEscalonado(Convert.ToInt32(txtQtdeInteiro.Text), CSPDVs.Current.PEDIDOS_PDV.Current.COD_EMPREGADO_ESCALONAMENTO, ((CSProdutos.CSProduto.CSPrecosProdutos.CSPrecoProduto)((CSItemCombo)cboTabelaPreco.SelectedItem).Valor).COD_TABELA_PRECO);

                    if (descontoMaximo > -1)
                    {
                        MessageBox.AlertErro(ActivityContext, string.Format("O percentual de desconto não pode ser maior que: {0}.", descontoMaximo.ToString(CSGlobal.DecimalStringFormat)));
                        return false;
                    }
                    else
                        return true;
        ...
        private static void SalvaDados()
        {
            try
            {
                    ...
                    itempedido.STATE == ObjectState.NOVO_ALTERADO)
                {
                    ...
                    if (ProdutoAbatimento.txtQtdeUnidadeIndenizacao != null)
                        itempedido.QTD_INDENIZACAO_UNIDADE = CSGlobal.StrToInt(ProdutoAbatimento.txtQtdeUnidadeIndenizacao.Text);

                    if (ProdutoAbatimento.txtValorFinalItemIndenizacao != null &&
                        itempedido.QTD_INDENIZACAO_INTEIRA != 0 || itempedido.QTD_INDENIZACAO_UNIDADE != 0)
        ...
        private bool ValidaFormatacaoNumerica()
        {
            try
            {
                ...
                if (ProdutoAbatimento.txtValorFinalItemIndenizacao != null &&
                    ProdutoAbatimento.txtValorFinalItemIndenizacao.Text != string.Empty &&
        ...
        private static bool ValidaAbatimento()
        {
            ...
            {
                if (ProdutoAbatimento.txtQtdeInteiroIndenizacao != null &&
                    GetQtdPedidaUnidadeIndenizacao() > 0)
                {
        ...
        private static bool ValidacaoNaoBroker()
        {
            try
            {
                    ...
                    CSGlobal.StrToDecimal(txtDescIncond.Text) > 0)
                {
                    ...
                    if (descontoMaximo > -1)
                    {
                        if (CSGlobal.StrToDecimal(txtDescIncond.Text) > descontoMaximo)
                        {
                            MessageBox.AlertErro(ActivityContext, string.Format("O percentual de desconto não pode ser maior que o máximo escalonado: {0}.", descontoMaximo.ToString(CSGlobal.DecimalStringFormat)));
                            return false;
                        }
                        else
                            return true;
