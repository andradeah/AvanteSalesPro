...
namespace AvanteSales.Pro.Dialogs
{
    public class DialogMetaLinha : Android.Support.V4.App.DialogFragment
    {
        ...
        static bool ResumoPorEmpregado;
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            ResumoPorEmpregado = Arguments.GetBoolean("ResumoPorEmpregado");
            return view;
        }
        ...
        private class ThreadCarregarMetasVendas : AsyncTask
        {
            ...
            {
                if (ResumoPorEmpregado)
                    adapter = new CSGruposComercializacao.CSGrupoComercializacaoMetaVenda().RetornarMetaVendaGrupoComercializacaoVendedor();
                else
        ...
        private class ListMetasVendaAdapter : BaseAdapter
        {
            ...
            public override View GetView(int position, View convertView, ViewGroup parent)
            {
                ...
                TextView lblPorcentagemVenda = convertView.FindViewById<TextView>(Resource.Id.lblPorcentagemVenda);

                lblDescricaoLinha.Text = linhaAtual.DES_GRUPO_COMERCIALIZACAO;
                lblMeta.Text = linhaAtual.VLR_OBJETIVO == -1 ? "-" : (string.Format("{0:0,0.00}", linhaAtual.VLR_OBJETIVO));
                lblVendido.Text = linhaAtual.VLR_VENDIDO == -1 ? "-" : (linhaAtual.VLR_VENDIDO == 0 ? "0,00" : string.Format("{0:0,0.00}", linhaAtual.VLR_VENDIDO));
                lblPorcentagemVenda.Text = linhaAtual.PCT_VENDA == -1 ? "-" : string.Format("{0}",linhaAtual.PCT_VENDA.ToString(".0"));
